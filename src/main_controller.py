                ...
                try:
                    ...
                    if ecodes.EV_KEY in capabilities:
                        ...
                        if numpad_keys_found:
                            ...
                            logger.info(
                                f"Found numpad device: {device.name} ({device_path}) - supports {len(numpad_keys_found)} numpad keys")
                        else:
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--debug":
        ...
        logger.info("Running in DEBUG mode with debug logging")

    # Show user information
    sudo_user = os.environ.get('SUDO_USER')
    if sudo_user and os.geteuid() == 0:
        logger.debug(f"Running as sudo for user: {sudo_user}")
    else:
        logger.debug(f"Running as user: {os.environ.get('USER', 'unknown')}")
